...
const el = {
  ...
  singleHighScore: document.getElementById("singleHighScore") // always visible
};
...
// Load single player high score from localStorage
...
function slotAllowsPos(slot,pos){ const p=normalizePos(pos); return slot==="FLEX"?FLEX_ALLOWED.has(p):slot===p; }
function rosterFilled(roster){ return roster.every(r=>!!r.player); }
function activeRoster(){ return mode==="single"?roster1:(currentPicker===1?roster1:roster2); }
function nextOpenSlotIndex(roster,pos){ const p=normalizePos(pos); for(let i=0;i<roster.length;i++){ if(!roster[i].player && (roster[i].slot===p)) return i; } if(FLEX_ALLOWED.has(p)){ for(let i=0;i<roster.length;i++){ if(!roster[i].player && roster[i].slot==="FLEX") return i; } } return -1; }
function canPickForRoster(roster,player){ return nextOpenSlotIndex(roster,player.pos)!==-1; }
function isPickLegal(player){ if(!currentTeam) return false; if(teamCode(player.team)!==teamCode(currentTeam)) return false; const roster=activeRoster(); const sel=getSelectedSlot(); if(sel!==null){ const row=roster[sel]; if(!row||row.player) return false; return slotAllowsPos(row.slot,player.pos); } return canPickForRoster(roster,player); }
...
function startGame(nextMode){
  ...
  el.rostersOne.classList.toggle("hidden", mode!=="single");
  el.rostersTwo.classList.toggle("hidden", mode==="single");
...
function buildTeamListFromRemaining(){
  const s=new Set();
  for(const p of remainingPlayers){ const t=teamCode(p.team); if(t) s.add(t); }
...
function startTimer(){ stopTimer(); timerHandle=setInterval(()=>{ timerLeft--; renderHeader(); if(timerLeft<=0) handleTimerExpire(); },1000);}
...
function renderHeader(){
  ...
  if(lf){ el.teamLogo.src=lf; el.teamLogo.style.display="block"; }
  else{ el.teamLogo.removeAttribute("src"); el.teamLogo.style.display="none"; }
...
function renderRoster(targetEl, roster, isActive){
  targetEl.innerHTML = roster.map((r,i)=>{
    ...
    return `<div class="slot-row ${selected}" data-idx="${i}">
...
function renderPlayers(){
  ...
  const list=remainingPlayers.filter(matchesFilters);
  list.sort((a,b)=>pts(b)-pts(a));

  el.playersBody.innerHTML=list.map(p=>{
    const legal=isPickLegal(p);
    return `<tr class="${legal?"":"illegal"}" data-id="${p.id}">
      <td>${p.name}</td>
...
function renderAll(){
  renderHeader();
  if(mode==="single") renderRoster(el.p1Slots, roster1, true);
  else{
...
(async function init(){
  ...
  try{
    ...
    remainingPlayers=[...allPlayers];
    el.emptyMsg.classList.add("hidden");
    // Show stored high score on init
    if(el.singleHighScore) el.singleHighScore.textContent=`High Score: ${singlePlayerHighScore}`;
    showModeScreen();
  } catch(err){
    console.error(err);
    showModeScreen();
